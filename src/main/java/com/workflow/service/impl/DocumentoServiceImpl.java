...
public class DocumentoServiceImpl implements DocumentoService {
    ...
    public Documento gestionarColaboradores(String id, String colaborador, String accion, String usuario) {
        ...
        if ("AGREGAR".equalsIgnoreCase(accion)) {
            if (!documento.getColaboradores().contains(colaborador)) {
                documento.getColaboradores().add(colaborador);
                log.info("Colaborador '{}' agregado al documento '{}' por '{}'", colaborador, documento.getNombre(), usuario);
                registrarEventoEnSolicitud(documento.getSolicitudId(), usuario,
                        String.format("Colaborador '%s' agregado al documento '%s'", colaborador, documento.getNombre()));
            }
        } else if ("QUITAR".equalsIgnoreCase(accion)) {
            documento.getColaboradores().remove(colaborador);
            log.info("Colaborador '{}' quitado del documento '{}' por '{}'", colaborador, documento.getNombre(), usuario);
            registrarEventoEnSolicitud(documento.getSolicitudId(), usuario,
                    String.format("Colaborador '%s' removido del documento '%s'", colaborador, documento.getNombre()));
        }
